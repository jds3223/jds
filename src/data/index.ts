import { Property } from "@/types/types";
export const locations = ["Business Bay", "Al Jaddaf", "Dubai Marina", "Al Sufouh 1"];
export const properties: Property[] = [
  {
    id: 1,
    title: "The Terraces Marasi Drive",
    image: "/assets/images/exclusives/SEkrPeeIie.webp",
    location: "Business Bay",
    locationLabel: "Business Bay",
    features: [
      { title: "Business Spot", description: "Perfect residence for businessmen and executives" },
      { title: "Comfortability", description: "Resort-style living amenities and premium services" },
      { title: "Property Range", description: "Range of apartments, duplexes, and penthouses" },
      { title: "View", description: "Breathtaking view of Burj Khalifa" }
    ],
    description:
      "Location on Marasi Drive provides quick access to main road arteries. 20-minute walk to Burj Khalifa and the Dubai Mall. Jumeirah Beach and Dubai International Airport are just 15 minutes away.",
    handoverDate: "June 30, 2024"
  },
    {
    id: 1,
    title: "The Terraces Marasi Drive",
    image: "/assets/images/exclusives/SEkrPeeIie.webp",
    location: "Al Sufouh 1",
    locationLabel: "Al Sufouh 1",
    features: [
      { title: "Business Spot", description: "Perfect residence for businessmen and executives" },
      { title: "Comfortability", description: "Resort-style living amenities and premium services" },
      { title: "Property Range", description: "Range of apartments, duplexes, and penthouses" },
      { title: "View", description: "Breathtaking view of Burj Khalifa" }
    ],
    description:
      "Location on Marasi Drive provides quick access to main road arteries. 20-minute walk to Burj Khalifa and the Dubai Mall. Jumeirah Beach and Dubai International Airport are just 15 minutes away.",
    handoverDate: "June 30, 2024"
  },
    {
    id: 1,
    title: "The Terraces Marasi Drive",
    image: "/assets/images/exclusives/SEkrPeeIie.webp",
    location: "Dubai Marina",
    locationLabel: "Dubai Marina",
    features: [
      { title: "Business Spot", description: "Perfect residence for businessmen and executives" },
      { title: "Comfortability", description: "Resort-style living amenities and premium services" },
      { title: "Property Range", description: "Range of apartments, duplexes, and penthouses" },
      { title: "View", description: "Breathtaking view of Burj Khalifa" }
    ],
    description:
      "Location on Marasi Drive provides quick access to main road arteries. 20-minute walk to Burj Khalifa and the Dubai Mall. Jumeirah Beach and Dubai International Airport are just 15 minutes away.",
    handoverDate: "June 30, 2024"
  },
    {
    id: 1,
    title: "The Terraces Marasi Drive",
    image: "/assets/images/exclusives/SEkrPeeIie.webp",
    location: "Al Jaddaf",
    locationLabel: "Al Jaddaf",
    features: [
      { title: "Business Spot", description: "Perfect residence for businessmen and executives" },
      { title: "Comfortability", description: "Resort-style living amenities and premium services" },
      { title: "Property Range", description: "Range of apartments, duplexes, and penthouses" },
      { title: "View", description: "Breathtaking view of Burj Khalifa" }
    ],
    description:
      "Location on Marasi Drive provides quick access to main road arteries. 20-minute walk to Burj Khalifa and the Dubai Mall. Jumeirah Beach and Dubai International Airport are just 15 minutes away.",
    handoverDate: "June 30, 2024"
  },
];


export const options = [
  {
    label: 'BUY',
    image: '/assets/images/exclusives/SEkrPeeIie.webp',
    describtion:'With a comprehensive portfolio of properties and countless offers, we cover all your real estate needs.'
  },
  {
    label: 'RENT',
    image: '/assets/images/exclusives/SEkrPeeIie.webp',
    describtion:'With a comprehensive portfolio of properties and countless offers, we cover all your real estate needs.'
  },
  {
    label: 'SELL',
    image: '/assets/images/exclusives/SEkrPeeIie.webp',
    describtion:'With a comprehensive portfolio of properties and countless offers, we cover all your real estate needs.'
  },
];